...
export const BentoGrid = ({
  ...
  return (
    <div
      className={cn(
        "mx-auto w-full max-w-7xl px-4 grid grid-cols-1 gap-4 md:auto-rows-[8rem] lg:auto-rows-[10rem] md:grid-cols-4 lg:grid-cols-6",
        className,
      )}
    >
      {children}
    ...
    >
      <div className={id === 6 ? 'flex justify-center h-full' : ''}>
        ...
        <div className={cn(titleClassName, 'group-hover/bento:translate-x-2 transition duration-200 relative md:h-full flex flex-col px-5 p-5 lg:p-10')}>
          <div className="font-inter text-sm md:text-xs lg:text-base z-10 font-extralight text-[#c1c3c3] dark:text-neutral-300">
            {description}
          </div>
          <div className="font-inter font-bold text-lg lg:text-3xl max-w-96 z-10">
            {title}
          </div>

          {/* {id === 2 && <Globe />} for globe  */}
          {id === 3 && (
